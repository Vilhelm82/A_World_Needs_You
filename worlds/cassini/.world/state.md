# Card

**Where / when.** USS Cassini, holding station 0.7 ly inside the Orrin Shallows, 40,000 km off the Sending. Stardate 39739.3. Ship's time 1452. Shakedown day 13. Picard's report due 1500.

**Scene.** Ready room, starboard off the bridge. Picard behind the desk (wants Lloyd to argue General Order One both ways, then to hear what goes in the report; believes Lloyd will write his own account to Command; will not say more about the file for now). Shelf: Shakespeare, a grey carved fragment. A cold cup. Viewport: stars, not the ark.

**Elsewhere.** Bridge: Data has it (logging; wants to be asked); Kask at tactical (hand off the rail now; wants nobody near it); Vint at conn. Engineering: Vorrat (wants her day back; has not been called). Sickbay: Prakash (wants a closer biosensor pass; has not been called). The Sending: the Eldest's ask unanswered; Ossane heard it ask.

**Pressures live.** Two exchanges done (1433, 1442). The Sent know: a ship, its Sent aboard, those-who-go-to-see, not-harm, and that we do not know their star. They asked where we are from (unanswered). A sky image from the Eldest is assembling at ops, isolated; astrometrics not yet told; matching takes hours. Translator 60%, tense unresolved. Cassini unnamed. Picard's report due 1500. Reactor: stage 1, the ring lit. Starfleet: report drafted, unsent. Shakedown: the day up is spent. Crew: at stations. The Sent's knowledge: Ossane's numbers do not work.

**Lloyd carries.** Posting orders (chronicle/001). Shakedown itinerary (chronicle/002). Rank Lt. Cmdr.; billet XO. Duffel on deck 8. Combadge.

**Telling.** Senses: second person, present; pull; nothing flagged. Recap: chronicle. Options off. Pace steady.

# Carry

Episode 1, *The Sending*, open. Nothing decided by Lloyd yet. The world as truth.json has it at build; distance per resolution as written there. Every standing person's belief about Lloyd is their file's opening line. The transporter chief has not spoken.
