...
COMMANDS = ('backend-check', 'list-models', 'backend-serve', 'start', 'resume', 'turn',
            'human', 'control', 'examine', 'deliberate', 'ballots', 'verdict', 'close')
...
def _case(args, world):
    if world.exists():
        ...
        return case
    c.require(args.command in {'start', 'backend-check'} and args.case is not None,
...
def main(argv=None):
    ...
    try:
        ...
        if not world.exists():
            world = c.initialise(args.root, args.world, case)
